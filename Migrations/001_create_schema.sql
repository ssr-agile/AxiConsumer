...
--   {schema}       => safely quoted identifier, e.g. "tenant001"
--   {schema_name}  => safely quoted string literal, e.g. 'tenant001'
--   {user_password} => safely quoted string literal supplied at runtime
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_roles
        WHERE rolname = v_schema
    ) THEN
        EXECUTE format('CREATE ROLE %I LOGIN PASSWORD %L', v_schema, v_password);
    ELSE
        -- Keep provisioning idempotent and allow password rotation/re-run during onboarding.
